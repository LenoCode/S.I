...
import static socket_installer_test_environment.socket_parts.notifications.extra_data_in_buffer_notification.ExtraDataInBufferNotification.EXTRA_DATA_IN_BUFFER_01_SERVER;
...
public class ExtraDataInBufferTestClient {

    @Rule
    public ClientResource clientResource = new ClientResource( new DataTradeModel[]{(DataTradeModel) new ExtraDataInBufferNotification()});
...
//            clientResource.getClientCreatedSocket().runSocket(EXTRA_DATA_IN_BUFFER_CLASS_IDENT,EXTRA_DATA_IN_BUFFER,"test");
    ...
    @Test
    public void checkIfDataisSendAndRecvCorrectly() throws IOException, SocketExceptions {
        asyncCommunicator.initNewThread((long) 10000);
        asyncCommunicator.addParameterizedObject("Counter",0);
        clientResource.getClientCreatedSocket().runSocket(EXTRA_DATA_IN_BUFFER_CLASS_IDENT,EXTRA_DATA_IN_BUFFER_01_SERVER,"1");

        asyncCommunicator.waitForFlag((long) 10000,"TestFinished");
        Integer counter = asyncCommunicator.getParameterizedObject("Counter");
        sleep(5000);
        Assertions.assertThat(counter).isEqualTo(1000);
